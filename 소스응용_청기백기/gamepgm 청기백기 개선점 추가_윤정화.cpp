	...
	do
	{
		...
		switch (choice)
		{
			case 1:
			{
				...
				{
					printf("\n  %d번 참가자의 이름을 입력하고 Enter>", i+1);
					scanf("%s", &user_name[i]);
				}
				printf("\n  만약 한글을 사용하였다면 영문키로 바꾸십시오.\n");
				printf("  아무키나 누르시오.\n");
				getch();
			   printf("\n[게임의 순서는 다음과 같습니다.] \n");
			   for(i=0;i<user_number;i++)
		...
		case 2:
				printf("\n [게임 설명 및 규칙] \n\n");
                printf("\n 주어진 문구를 확인하여 알맞은 키를 누르기만 하는 게임입니다. \n");
                printf("\n 문제당 키는 한번만 눌러주세요. \n");
                printf("\n\n 메인화면으로 돌아가려면 아무키나 눌러주세요. \n");
                getch();
                break;
        ...
        case 3:
		       printf("게임을 종료합니다.\n");
                break;
       default:
                printf("잘못된 선택입니다. 종료 후 재실행 해주세요. \n");
                break;
		}
	} while (choice !=3);
